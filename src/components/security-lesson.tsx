...
export function SecurityLesson({ topicId }: { topicId: string }) {
  ...
  return <>
    ...
    <Sources><ul>{lesson.sources.map((source) => <li key={source.url}><a href={source.url} rel="noreferrer">{source.label}</a></li>)}</ul><p><small>Reviewed 18 September 2026. Educational material only; re-check current standards, threats, and jurisdictional requirements.</small></p></Sources>
  </>;
}
